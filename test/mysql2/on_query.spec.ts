...
describe('[mysql2]: on query', () => {
  ...
  beforeEach(async () => {
    mockOnQuery = jest.fn();
    ({ rollback } = await startTransaction({ onQuery: mockOnQuery }));
  });
  ...
  it('insert', async () => {
    ...
    expect(mockOnQuery).toHaveBeenCalledTimes(2);
  });
  ...
  it('update', async () => {
    ...
    expect(mockOnQuery).toHaveBeenCalledTimes(3);
  });
  ...
  it('insert: commit', async () => {
    ...
    expect(mockOnQuery).toHaveBeenCalledTimes(4);
  });
  ...
  it('insert: two parallel transcation, one commit, one rollback', async () => {
    ...
    expect(mockOnQuery).toHaveBeenCalledTimes(8);
  });
});
